...
stateDiagram-v2
  ...
  CaptureRisk --> Reform
  Reform --> Adaptation
  CaptureRisk --> LegitimacyCrisis
  LegitimacyCrisis --> Emergency
  Emergency --> Reconstitution
  Emergency --> Collapse
  Reconstitution --> Consolidation
  Collapse --> Fragmentation
  Fragmentation --> SuccessorFormation
```
...
stateDiagram-v2
  [*] --> StableComplexity
  StableComplexity --> RisingStress
  RisingStress --> BufferDrawdown
  BufferDrawdown --> InstitutionalStrain
  InstitutionalStrain --> LegitimacyCrisis
  InstitutionalStrain --> AdaptiveReform
  AdaptiveReform --> StableComplexity
  LegitimacyCrisis --> CascadingFailure
  CascadingFailure --> Simplification
  Simplification --> Fragmentation
  Fragmentation --> Recovery
  Fragmentation --> DarkAge
...
```mermaid
graph LR
  Energy["Energy constraint"] --> Food["Food production stress"]
  Energy --> Industry["Industrial throughput decline"]
  Food --> Prices["Price instability"]
  Prices --> Unrest["Social unrest"]
  Industry --> Fiscal["Fiscal distress"]
  Fiscal --> Services["Public service degradation"]
  Services --> Trust["Institutional trust decline"]
  Trust --> Compliance["Compliance decline"]
  Compliance --> Fiscal
  Trust --> Epistemic["Epistemic fragmentation"]
  Epistemic --> Coordination["Coordination failure"]
...
stateDiagram-v2
  ...
  Evaluated --> Deployed
  Deployed --> Monitored
  Monitored --> DriftSuspected
  DriftSuspected --> Quarantined
  DriftSuspected --> Cleared
  Quarantined --> Remediated
  Remediated --> Reevaluated
  Reevaluated --> Deployed
  Quarantined --> Retired
```
...
```mermaid
graph TD
  Now["2026 baseline"] --> A["Managed transition"]
  Now --> B["Fragmented acceleration"]
  Now --> C["Polycrisis stagnation"]
  Now --> D["Authoritarian stabilization"]
  Now --> E["Systemic collapse"]
  A --> A1["Human-AI constitutional civilization"]
  A --> A2["Ecological-technological steady state"]
  B --> B1["Platform sovereignties"]
  B --> B2["AI arms-race instability"]
  C --> C1["Regional resilience islands"]
  C --> C2["Institutional decay spiral"]
  D --> D1["High-control technocracy"]
  D --> D2["Legitimacy rupture"]
  E --> E1["Localized recovery"]
  E --> E2["Knowledge discontinuity"]
```
...
graph TD
  ...
  Civilization --> KnowledgeSystem
  Civilization --> Infrastructure
  Civilization --> Ecology
  Civilization --> AISystem
  Institution --> Constitution
  Institution --> GovernanceProcess
  Institution --> Office
  GovernanceProcess --> Decision
  Decision --> Evidence
  Evidence --> Claim
  Claim --> KnowledgeSystem
  KnowledgeSystem --> Archive
  Archive --> Artifact
  Economy --> Resource
  Economy --> AllocationMechanism
  AISystem --> AlignmentState
  AISystem --> OversightProcess
  Ecology --> Boundary
...
- AI risk management: NIST AI RMF 1.0 provides a risk%2Dmanagement framework for trustworthy and responsible AI. Source: [NIST AI RMF 1.0](https://www.nist.gov/publications/artificial-intelligence-risk%2Dmanagement-framework-ai-rmf-10).
