...
export async function createOrderAction(input: unknown) {
  ...
  const { error } = await supabase.from("orders").insert(parsed.data as any);
...
export async function updateOrderStatusAction(orderId: string, input: unknown) {
  ...
  const { error } = await supabase.from("orders").update(orderUpdate as any).eq("id", orderId);
  ...
  await supabase.from("order_status_history").insert(historyInsert as any);
