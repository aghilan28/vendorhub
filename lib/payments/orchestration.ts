...
export async function requestAndInitiateRefund(input: unknown) {
  ...
  if (attemptError || !attempt) {
    await admin.from("refund_requests").update({ state: "REFUND_RECONCILING", failure_reason: "PAYMENT_ATTEMPT_NOT_FOUND" }).eq("id", refundId);
  ...
  if (!paymentAttempt.provider_payment_id) {
    await admin.from("refund_requests").update({ state: "REFUND_RECONCILING", failure_reason: "PROVIDER_PAYMENT_ID_MISSING" }).eq("id", refundId);
  ...
  try {
    await admin.from("refund_requests").update({ state: "REFUND_INITIATED" }).eq("id", refundId);
    ...
    if (refund.status === "processed") {
      ...
      if (adjustmentError) {
        await admin.from("refund_requests").update({ state: "REFUND_RECONCILING", failure_reason: "REFUND_ACCOUNTING_ADJUSTMENT_FAILED" }).eq("id", refundId);
    ...
    await admin
      .from("refund_requests")
      .update({ state: "REFUND_FAILED", failure_reason: error instanceof Error ? error.message : "Razorpay refund initiation failed." })
      .eq("id", refundId);
