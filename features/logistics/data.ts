...
function buildDelivery(status: DeliveryStatus = "IN_TRANSIT"): Delivery {
  ...
  return {
    ...
    events: [
      { id: "evt-del-1042-created", deliveryId: `del-${order.id}`, status: "DELIVERY_PENDING", type: "dispatch_created", title: "Order accepted", description: "Seller accepted the order and started packing.", occurredAt: eventTimes.created, actor: "seller", locationLabel: product.vendor.locality, etaMinutes: eta.estimatedMinutes },
      { id: "evt-del-1042-assigned", deliveryId: `del-${order.id}`, status: "READY_FOR_DISPATCH", type: "self_delivery_assigned", title: "Delivery partner assigned", description: "A local delivery partner has been assigned for pickup.", occurredAt: eventTimes.assigned, actor: "seller", locationLabel: product.vendor.locality, etaMinutes: eta.estimatedMinutes },
