...
export async function createSelfDelivery(orderId: string, config: SelfDeliveryConfig) {
  ...
  const { data: delivery, error } = await supabase
    .from("deliveries")
    .insert({ order_id: orderId, buyer_id: order?.buyer_id, vendor_id: order?.vendor_id, mode: "SELLER_SELF", status: "PENDING_DISPATCH", eta_minutes: config.etaHours * 60 } as any)
    .select("id")
    .single();
...
export async function updateSelfDeliveryStatus(deliveryId: string, status: string, note: string) {
  ...
  await supabase.from("delivery_tracking_events").insert({ delivery_id: deliveryId, status: normalizedStatus, event_type: "seller_self", title: "Delivery update", body: note, actor_type: "seller" } as any);
