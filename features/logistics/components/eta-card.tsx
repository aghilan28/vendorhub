    ...
    <section className="rounded-lg border border-border bg-surface p-4 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Badge variant="default">
          <Clock3 className="size-3" /> {delivery.etaWindow}
        </Badge>
      </div>
      <h2 className="mt-3 text-2xl font-semibold text-primary-text">{delivery.orderCode}</h2>
      <p className="mt-2 text-sm leading-6 text-secondary-text">
        {delivery.partner.name} from a nearby seller, {delivery.distanceKm.toFixed(1)} km away. Expected by{" "}
      ...
      <div className="mt-4 grid gap-2 text-sm">
        <span className="flex items-center gap-2 rounded-md bg-slate-50 px-3 py-2 text-secondary-text">
          <Navigation className="size-4 text-emerald-700" /> {delivery.assignedTo ?? "Delivery partner being assigned"} {delivery.assignedPhone ? `- ${delivery.assignedPhone}` : ""}
        </span>
        <span className="flex items-center gap-2 rounded-md bg-slate-50 px-3 py-2 text-secondary-text">
          <ShieldCheck className="size-4 text-emerald-700" /> We will keep this page updated until delivery.
        </span>
      </div>
    </section>
  );
