  ...
  return (
    <section className="rounded-lg border border-border bg-surface p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-medium uppercase text-secondary-text">Seller legitimacy</p>
          <h2 className="mt-1 font-semibold text-primary-text">{vendor.name}</h2>
          <p className="mt-1 text-sm text-secondary-text">{profile.businessType.replaceAll("_", " ")} · {profile.city}</p>
        </div>
