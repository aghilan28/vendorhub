...
export function ModerationScreen({ type }: { type?: "product" | "review" }) {
  ...
  return <GovernanceTable title={type === "product" ? "Product moderation queue" : type === "review" ? "Review moderation queue" : "Moderation command queue"} description="Structured, auditable moderation with status, priority, history, visibility toggles, and escalation placeholders." rows={rows} columns={columns} empty={<EmptyState icon={ShieldCheck} title="No moderation queue" description="No cases match the current moderation filters." />} actions={<div className="flex gap-2"><Select value={status} onValueChange={(value) => setStatus(value as never)}><SelectTrigger className="w-44"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All states</SelectItem><SelectItem value="pending_review">Pending review</SelectItem><SelectItem value="approved">Approved</SelectItem><SelectItem value="rejected">Rejected</SelectItem><SelectItem value="flagged">Flagged</SelectItem><SelectItem value="suspended">Suspended</SelectItem></SelectContent></Select><Button variant="secondary"><Archive /> Bulk placeholder</Button></div>} />;
}
