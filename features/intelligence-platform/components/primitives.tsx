      ...
      <section className="operational-surface rounded-lg p-4 sm:p-5">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div className="min-w-0">
            <p className="text-xs font-medium uppercase tracking-wide text-secondary-text">KARTEX Commerce Intelligence Platform</p>
...
export function NavCard({ href, label, icon: Icon, external }: { href: string; label: string; icon: LucideIcon; external?: string }) {
  return (
    <Link href={href as Route} className="flex items-center gap-3 rounded-md border border-border p-3 focus-ring hover:bg-slate-50">
      ...
      <span className="min-w-0">
        ...
        {external ? <span className="block text-[11px] text-secondary-text">{external}</span> : null}
      </span>
    </Link>
  );
}

export function SystemPill({ system }: { system: string }) {
  const label = system === "secis" ? "SECIS" : system.charAt(0).toUpperCase() + system.slice(1);
